...
def main():
    parser = argparse.ArgumentParser(description='Train RoadSignNet-SAL Optimized V4')
    ...
    print("=" * 70)
    print("ROADSIGNNET-SAL OPTIMIZED V4 TRAINING")
    print("With Data Augmentation - Full Training (No Early Stopping)")
    print("=" * 70)
    ...
    # Data loaders - AUGMENTATION ENABLED
    ...
    print("\n" + "=" * 70)
    print("STARTING TRAINING (Full {args.epochs} epochs)")
    print("=" * 70)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # Save best model (based on val_loss, but we'll evaluate all checkpoints)
        if val_metrics['total_loss'] < best_val_loss:
            ...
            }, 'outputs/checkpoints/best_model_optimized_v4.pth')
            print(f"✓ New best val_loss model saved! (val_loss: {best_val_loss:.4f})")
        
        # Save checkpoints every 10 epochs for later mAP evaluation
        if epoch % 10 == 0:
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                ...
                'width_multiplier': args.width,
                'model_type': 'optimized'
            }, f'outputs/checkpoints/checkpoint_optimized_v4_epoch_{epoch}.pth')
            print(f"✓ Saved checkpoint: checkpoint_optimized_v4_epoch_{epoch}.pth")
        
        # Save history
        with open('outputs/logs/optimized_v4_training.json', 'w') as f:
            for h in history:
    ...
    torch.save({
        ...
        'width_multiplier': args.width,
        'model_type': 'optimized'
    }, 'outputs/checkpoints/final_model_optimized_v4.pth')
    
    print("\n" + "=" * 70)
    print("TRAINING COMPLETE")
    print("=" * 70)
    print(f"Best validation loss: {best_val_loss:.4f}")
    print(f"\nCheckpoints saved for evaluation:")
    for e in range(10, args.epochs + 1, 10):
        print(f"  - checkpoint_optimized_v4_epoch_{e}.pth")
    print(f"  - final_model_optimized_v4.pth")
    print(f"\nTo evaluate all checkpoints, run:")
    print(f"  python scripts/evaluate.py --checkpoint outputs/checkpoints/checkpoint_optimized_v4_epoch_XX.pth")
